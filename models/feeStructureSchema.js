...
const feeStructureSchema = new mongoose.Schema({
  ...
  collectionMonths: {
    type: [String],
    validate: {
      validator: arr => arr.length > 0 && arr.every(m => MONTHS.includes(m)),
...
feeStructureSchema.pre("validate", function(next) {
  if (this.collectionMonths?.length && typeof this.amountPerInstallment === "number") {
...
feeStructureSchema.pre("validate", function(next) {
  ...
  if (months.size === 0 || months.size !== keys.size) {
    return next(new Error("dueDates must include exactly one date for each collection month."));
  }
  for (const m of months) if (!keys.has(m)) return next(new Error(`Missing dueDate for month: ${m}`));
  next();
});
